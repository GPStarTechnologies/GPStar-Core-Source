 ...
 * https://github.com/GPStarTechnologies/GPStar-Core-Source/blob/main/docs/WAVTRIGGER.md
 */
 ...
 * Audio Devices
 */
enum AUDIO_DEVICES { AUDIO_NONE, AUDIO_GPSTAR, AUDIO_GPSTAR_ADV, AUDIO_WAV_TRIGGER };
...
void playMusic() {
  if(i_music_track_count > 0 && i_current_music_track >= i_music_track_start) {
    ...
    wandSerialSend(A_MUSIC_STATUS, b_playing_music ? 2 : 1);
...
void pauseMusic() {
  if(b_playing_music && !b_music_paused) {
    ...
    wandSerialSend(A_MUSIC_STATUS, b_music_paused ? 4 : 3);
...
bool setMasterVolumePercentage(uint8_t percentage) {
  ...
  i_volume_master = getGainValue(percentage);
...
bool setupAudioDevice() {
  ...
  if(audio.getVersion(gVersion)) {
    ...
    AUDIO_DEVICE = AUDIO_WAV_TRIGGER;
