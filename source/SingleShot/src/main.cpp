...
void setup() {
  // Initialize both LED chains
  LightingManager::getInstance(CHAIN_SYSTEM).initializeDriver();
...
void animateTaskCallback() {
  ...
  LightingManager::getInstance(CHAIN_SYSTEM).show();
  LightingManager::getInstance(CHAIN_VENT).show();
