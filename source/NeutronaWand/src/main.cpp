  ...
  if(WAND_ACTION_STATUS != ACTION_FIRING) {
    if(ms_bmash.remaining() < 1) {
      ...
      if(b_wand_mash_lockout) {
        ...
        packSerialSend(A_MASH_ERROR_RESTART);
      ...
      if(b_pack_alarm) {
        ...
        if(getNeutronaWandYearMode() == SYSTEM_AFTERLIFE || getNeutronaWandYearMode() == SYSTEM_FROZEN_EMPIRE) {
          if(ms_gun_ramp_1.justFinished() && !switch_vent.on()) {
            if(b_extra_pack_sounds) {
              packSerialSend(A_AFTERLIFE_GUN_LOOP_1);
            }
  ...
  switch(WAND_CONN_STATE) {
    ...
    case PACK_CONNECTED:
      ...
      if(ms_handshake.justFinished()) {
        packSerialSend(A_HANDSHAKE, PROTOCOL_SIGNATURE); // Remind the pack that a wand is still present.
