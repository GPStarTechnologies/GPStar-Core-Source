...
# Purpose: Batch-burn required eFuses for ESP32-S3-WROOM to enable additional GPIO usage.
#
# ---
#
...
print_efuse_status() {
  echo "Current eFuse values for relevant eFuses:"
  $ESPEFUSE_BIN --port "$PORT" summary | grep -E 'UART_PRINT_CONTROL|VDD_SPI_FORCE|VDD_SPI_TIEH|DIS_PAD_JTAG' || echo "  (Could not parse efuse values)"
}
...
FORCE_VAL=$(echo "$summary_output" | grep 'VDD_SPI_FORCE' | awk '{print $NF}' | tr -d '()')
VDDSPI_VAL=$(echo "$summary_output" | grep 'VDD_SPI_TIEH' | awk '{print $NF}' | tr -d '()')
JTAG_VAL=$(echo "$summary_output" | grep 'DIS_PAD_JTAG' | awk '{print $NF}' | tr -d '()')

if [[ "$UART_VAL" =~ ^(3|0b11)$ ]] && [[ "$FORCE_VAL" =~ ^(1|0b1|BURNED)$ ]] && [[ "$VDDSPI_VAL" =~ ^(1|0b1|BURNED)$ ]] && [[ "$JTAG_VAL" =~ ^(1|0b1|BURNED)$ ]]; then
  echo "The eFuses are already set (UART_PRINT_CONTROL=$UART_VAL, DIS_PAD_JTAG=$JTAG_VAL). Skipping burn."
  exit 0
fi

echo "This script will permanently burn the following eFuses on your ESP32-S3-N16R8:"
echo "  - UART_PRINT_CONTROL = 3 (disables UART0 boot debug, enables GPIO46)"
echo "  - VDD_SPI_FORCE = 1 (Forces VDD_SPI to use eFuse value, enables GPIO45)"
echo "  - VDD_SPI_TIEH = 1 (Sets VDD_SPI to use VDD3P3_RTC for power, as default)"
echo "  - DIS_PAD_JTAG = 1 (disables JTAG on GPIO39-42, enables GPIO3)"
...
BURN_CMD=("$ESPEFUSE_BIN" --chip esp32s3 --do-not-confirm --port "$PORT" burn-efuse UART_PRINT_CONTROL 3 VDD_SPI_FORCE 1 VDD_SPI_TIEH 1 DIS_PAD_JTAG 1)
